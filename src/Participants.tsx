...
const events = [
  {
    ...
    description:
      "Join us for a 24-hour coding marathon where teams compete to build innovative solutions. Great prizes, amazing networking opportunities.",
    date: "Mar 15, 2024",
    time: "09:00 AM",
      ...
      <main className="p-6">
        ...
        {events.map((event) => {
          ...
          return (
            <div
              key={event.id}
              className="bg-[#2a2a2a] rounded-lg overflow-hidden mb-6"
            >
              {/* Event Header */}
              <div className="p-6 border-b border-[#333]">
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="bg-yellow-600/20 text-yellow-500 px-3 py-1 rounded-full flex items-center">
                    <DollarSign className="h-4 w-4 mr-1" />${event.prizeMoney}
                  </div>
                </div>
                ...
                {activeTab === "participants" && (
                  <div className="space-y-4">
                    {event.participants.map((participant, index) => (
                      <div
                        key={index}
                        className="flex items-center gap-4 p-3 bg-[#222] rounded-lg hover:bg-[#333] transition-colors"
                      >
                        <img
